...
def test_gpu_aware_scheduler_reserves_specific_gpu_uuids_for_a_lease():
    observation = GpuHostObservation(
        ...
        gpus=tuple(GpuDeviceObservation(i, f"GPU-{i}", "NVIDIA Test GPU", 81920, 0, f"00000000:{17+i:02x}:00.0", "10.0") for i in range(2)),
...
def test_gpu_aware_scheduler_releases_gpu_uuids_on_completion():
    observation = GpuHostObservation("worker-a", "580.95.05", "13.0", (GpuDeviceObservation(0, "GPU-0", "NVIDIA Test GPU", 81920, 0, "00000000:17:00.0", "10.0"),), "GPU0", False, None, None)
