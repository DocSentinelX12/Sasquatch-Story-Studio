...
def build_wan22_runtime_config(*, repository: Path, model_path: Path, output_path: str, python_executable: str = "python", timeout_seconds: int = 21600) -> VerifiedEngineRuntimeConfig:
    command = build_wan22_command(python_executable=python_executable, repository=repository, model_path=model_path, prompt="{prompt}", output_token="{output}")
    return VerifiedEngineRuntimeConfig(engine_id="wan2.2", command=command, output_path=output_path, working_directory=str(repository.expanduser().resolve()), timeout_seconds=timeout_seconds)


def build_ltx_video_runtime_config(*, repository: Path, model_path: Path, output_path: str, python_executable: str = "python", timeout_seconds: int = 21600) -> VerifiedEngineRuntimeConfig:
    command = build_ltx_video_command(python_executable=python_executable, repository=repository, model_path=model_path, prompt="{prompt}", image_path="{image_path}", output_token="{output}")
    return VerifiedEngineRuntimeConfig(engine_id="ltx-video", command=command, output_path=output_path, working_directory=str(repository.expanduser().resolve()), timeout_seconds=timeout_seconds)
