"""Small deterministic scheduler core for shared self-hosted capacity.

Persistence is deliberately kept outside this module so the same scheduling
logic can sit above SQLite, PostgreSQL, or another local durable store later.
"""
...
class Scheduler:
    ...
    def choose(self, worker_id: str, resources: ResourceSnapshot, now: int, lease_seconds: int = 900) -> Job | None:
        ...
        candidates = sorted(
            (j for j in self._jobs.values() if j.state == JobState.QUEUED),
