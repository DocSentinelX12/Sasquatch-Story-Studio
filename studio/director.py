...
def build_episode_plan(story_id: str, title: str, interpretation: dict[str, Any]) -> EpisodePlan:
    ...
    for scene_order, (scene_id, scene_events) in enumerate(scene_groups.items(), start=1):
        ...
        shots: list[Shot] = []
        for event_order, event in enumerate(scene_events, start=1):
            story_event = StoryEvent(
                id=event["id"], scene_id=scene_id, order=event_order,
            ...
            for character in event.get("characters", []):
                ...
                character_values.setdefault(character_id, CharacterRef(character_id, str(character.get("name", character_id)), character.get("role")))
            event_dialogue = tuple(item["id"] for item in dialogue if item.get("scene_id") == scene_id and item.get("event_id") == event["id"])
            if not event_dialogue:
                event_dialogue = tuple(item["id"] for item in dialogue if item.get("scene_id") == scene_id and item.get("order", 0) == event.get("order", 0))
            duration = max(0.1, float(event.get("duration_seconds", 2.0)))
            shots.append(Shot(
                id=f"{scene_id}-shot-{event_order:03d}", scene_id=scene_id, order=event_order,
                purpose=f"Depict canonical event {event['id']} without changing its meaning.",
                duration_seconds=duration, characters=tuple(character_values.values()),
                location_id=event.get("location_id"), required_events=(event["id"],), dialogue_ids=event_dialogue,
        ...
        scenes.append(Scene(
            id=scene_id, order=scene_order, description=str(scene_events[0].get("scene_description", "")),
            location_id=scene_events[0].get("location_id"), time_of_day=scene_events[0].get("time_of_day"),
            characters=tuple(character_values.values()), events=tuple(story_events), shots=tuple(shots),
        ))
    ...
    fidelity_notes = (f"source_interpretation_hash={content_hash(interpretation)}", "No unreviewed creative invention is canonical.")
    return EpisodePlan(id=f"episode-{story_id}", story_id=story_id, title=title, scenes=tuple(scenes), fidelity_notes=fidelity_notes, review_items=review_items)
