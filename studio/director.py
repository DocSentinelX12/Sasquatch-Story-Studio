...
def build_episode_plan(story_id: str, title: str, interpretation: dict[str, Any]) -> EpisodePlan:
    ...
    for scene_order, (scene_id, scene_events) in enumerate(scene_groups.items(), start=1):
        ...
        if unassigned and shots:
            first = shots[0]
            shots[0] = Shot(
                id=first.id,
                scene_id=first.scene_id,
                order=first.order,
                purpose=first.purpose + " Unmapped scene dialogue is preserved here pending production review.",
                duration_seconds=first.duration_seconds,
                characters=first.characters,
                location_id=first.location_id,
                required_events=first.required_events,
                dialogue_ids=tuple(first.dialogue_ids) + unassigned,
                camera=first.camera,
                animation=first.animation,
                sound=first.sound,
            )
            review_items.append(
                f"CREATIVE REVIEW REQUIRED: scene {scene_id} dialogue mapping is not explicit; preserved dialogue {unassigned} without changing text."
            )
