...
class WorkerControlServer:
    ...
    def get_artifact_chunk(self, path: str, authorization: str | None) -> bytes:
        ...
        try:
            ...
            raise ValueError("invalid artifact chunk index") from exc
        if chunk_index < 0:
            raise ValueError("invalid artifact chunk index")
        query = parse_qs(parsed.query, keep_blank_values=True)
        if len(query.get("size", [])) != 1 or len(query.get("offset", [])) != 1:
            raise ValueError("artifact chunk range requires exactly one offset and size")
        try:
        ...
        with source_path.open("rb") as handle:
            ...
            total = handle.tell()
            if offset >= total:
                raise ValueError("artifact chunk is outside the source object")
            handle.seek(offset)
            data = handle.read(requested_size)
            if len(data) != requested_size:
                raise IOError("artifact chunk could not be read at the requested size")
            return data
