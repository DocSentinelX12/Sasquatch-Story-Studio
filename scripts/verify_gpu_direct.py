...
def verify(output_path, command_text, *, worker_pair=None, gpu_pair=None, interface=None, now=None):
    ...
    if not worker_pair or len(worker_pair) != 2 or worker_pair[0] == worker_pair[1]:
        raise ValueError("an explicit distinct worker pair is required")
    if not gpu_pair or len(gpu_pair) != 2 or gpu_pair[0] == gpu_pair[1]:
        raise ValueError("an explicit distinct GPU pair is required")
    if not interface or not interface.strip():
        raise ValueError("an explicit network interface is required")
    ...
    if not all(gpu in gpu_uuids for gpu in gpu_pair):
        raise RuntimeError("GPU-direct evidence references a GPU UUID not observed on the verification host")
    ...
    evidence={"verification":"gpu_direct_network","recorded_at":recorded_at,"command":command,"worker_pair":list(worker_pair),"gpu_pair":list(gpu_pair),"interface":interface,"gpu_uuids":list(gpu_uuids),
...
def main():
    ...
    parser.add_argument("--output",default="gpu-direct-runtime-evidence.json")
    parser.add_argument("--command",required=True)
    parser.add_argument("--worker-pair",required=True)
    parser.add_argument("--gpu-pair",required=True)
    parser.add_argument("--interface",required=True)
    args=parser.parse_args()
    verify(args.output,args.command,worker_pair=tuple(args.worker_pair.split(",",1)),gpu_pair=tuple(args.gpu_pair.split(",",1)),interface=args.interface)
    return 0
