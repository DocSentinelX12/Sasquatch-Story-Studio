...
def install(engine_id: str) -> None:
    ...
    if engine_id == "opentoonz":
        ...
        run("cmake", "../sources", cwd=build, timeout=1800)
    ...
    if engine_id == "rhubarb-lip-sync":
        ...
        run("cmake", "--build", ".", "--config", "Release", "--parallel", str(max(2, os.cpu_count() or 2)), cwd=build, timeout=3600)
    ...
    if engine_id == "comfyui":
        ...
        run(sys.executable, "main.py", "--help", cwd=ROOT / engine_id / "ComfyUI", timeout=180)
        evidence(engine_id, "https://github.com/Comfy-Org/ComfyUI", ROOT / engine_id, [])
        return
    ...
    if engine_id == "wan2.1":
        ...
        run(sys.executable, "generate.py", "--help", cwd=root / "Wan2.1", timeout=180)
        evidence(engine_id, "https://github.com/Wan-Video/Wan2.1", root, models)
        return
    ...
    if engine_id == "wan2.2":
        ...
        run(sys.executable, "generate.py", "--help", cwd=root / "Wan2.2", timeout=180)
        evidence(engine_id, "https://github.com/Wan-Video/Wan2.2", root, models)
        return
    ...
    if engine_id == "ltx-video":
        ...
        evidence(engine_id, "https://github.com/Lightricks/LTX-Video", root, [model])
        return
    ...
    if engine_id == "ace-step-1.5":
        ...
        evidence(engine_id, "https://github.com/ace-step/ACE-Step-1.5", ROOT / engine_id, models)
        return
...
def main() -> int:
    ...
    install(args.engine)
    print(json.dumps({"engine_id": args.engine, "status": "installed", "runtime_verified": False}, indent=2))
    return 0
