...
def verify(output_path, *, executable="all_reduce_perf", now=None):
    ...
    evidence={"verification":"nccl_collective","recorded_at":recorded_at,"command":command,"gpu_uuids":list(gpu_uuids),
...
def main():
    parser=argparse.ArgumentParser(); parser.add_argument("--output",default="nccl-runtime-evidence.json"); parser.add_argument("--executable",default="all_reduce_perf"); args=parser.parse_args()
    verify(args.output,executable=args.executable); return 0
